...
if [[ "$VERSION" == v* ]]; then
  echo "Error: pass the version without the leading 'v' (got '${VERSION}'). Example: ./release.sh 0.1.3"
  exit 1
fi
...
if [[ "${CURRENT_VERSION}" == "${VERSION}" ]]; then
  ...
  git add pom.xml l402-java-core/pom.xml l402-java-spring-boot-starter/pom.xml l402-java-examples/pom.xml README.md
